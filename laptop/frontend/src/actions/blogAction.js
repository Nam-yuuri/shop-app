...
export const getAllBlogs = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      "http://localhost:5000/api/v1/admin/blogs"
    );
...
export const createBlog = (blogData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `http://localhost:5000/api/v1/admin/blog/new`,
      blogData,
      config
    );
...
export const updateBlog = (id, blogData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `http://localhost:5000/api/v1/admin/blog/${id}`,
      blogData,
      config
    );
...
export const newReviewBlog = (reviewData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `http://localhost:5000/api/v1/blogReview`,
      {
...
export const deleteReviewsBlog = (reviewId, blogId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(
      `http://localhost:5000/api/v1/blogReviews?id=${reviewId}&blogId=${blogId}`,
      config
    );
